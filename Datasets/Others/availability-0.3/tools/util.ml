...
(******************************
 * Brighten's OCaml Utilities
 *
 *
 * To do:
 *   - new distributions
 *   - "primed" distributions: functions that give you the distribution at
 *     a random time in [0,T] during a renewal process with distribution X,
 *     as T -> \infty
 *
 ******************************)
...
let stats ?(num_trials: int          = 1)
	...
	let print_header () =
		...
		f o "#\n# Created automatically by stats program.\n";
...
plot [] [] \\\n"
		...
		foreach_i var2domain (fun j (var2valname, var2val) ->
			...
			let linetype = sprintf "lt %d pt %d" color color in
		...
		if show_plots then
		    ignore (Sys.command ("gv \"" ^ base_plot_name ^ ".ps\""))
        )

let percentile p lst =
	let sorted_l = List.sort compare lst in
    List.nth sorted_l (int_of_float (p *. (fl (List.length sorted_l - 1))))	
 ...
 * in gnuplot format.
 * "Square"=true connects the dots in the correct way
 * "square"=false connects them directly with lines which sometimes looks better
 *)
let output_cdf_floats ?(square:bool=true) data file_path =
	let n = List.length data in
	let fl_n = i2f n in
	...
	let print_point x y = fprintf file "%g %g\n" x y in

	foreach data (fun d ->
		if d <> !last or !i = 0 then (
			if square or !i = 0 then
...
let output_cdf_ints data file_path =
	output_cdf_floats (List.map i2f data) file_path
...
class freq_counter = object(self)
	...
	method add_samples num_samples sample_value =
		let freq = 
			if Hashtbl.mem samples sample_value then
				Hashtbl.find samples sample_value
			else
				Int64.zero
			in
